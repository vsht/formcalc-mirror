...
\biitab
\Code{WeylME[\Var{\M_1},\,\Var{\M_2^*}]} &
	calculate the fermion matrix elements for all combinations
	of \Code{F\Vn} appearing inside \Code{Mat} in \Var{\M_1} and 
	\Var{\M_2^*}, where, as the $*$ indicates, \Var{\M_2} is the 
	part to be complex conjugated \\
\Code{WeylME[\Var{\M_1},\,\Var{\M_2^*},\,\Vf]} &
	the same, but additionally multiply each \Code{Mat[F\Vi,\,F\Vj]}
	with the correction factor \Code{\Vf[F\Vi,\,F\Vj]} \\
\Code{All} &
	(used as an argument of \Code{WeylME}:)
	instead of selecting the \Code{F}s which appear in an expression,
	simply take all \Code{F}s currently in the abbreviations
\etab
...
\begin{alltt}
        ...
        do 1 l = 1, \textrm{(\# of \Code{SUN}s in \Var{\M\sb{2}})}
          result = result + a(i,k)*Conjugate(b(j,l))*MatF(i,j)*MatSUN(k,l)
  1     continue
\end{alltt}
...
&	returns \Code{\Var{f_1}[\Var{expr}]} if 
	...
	is still larger \\
\Code{MapOnly[\Vh,\,\Var{syms}][\Vf,\,\Var{expr}]} &
	maps \Vf\ onto subexpressions of head \Vh\ in \Var{expr}
	which must contain one each of all items in \Var{syms} and no 
	other symbols \\
